...
| Camoufox-Browser (Anti-Bot) | 9377 | Anti-Detect-Firefox (ghcr.io/jo-inc/camofox-browser), nur Anti-Bot-Faelle (Cloudflare-artig), nicht Standard (Standard = Chrome-CDP 9222) | keyless (lokal 127.0.0.1) |
...
| ID | Gap | Status |
|---|---|---|
| OPS-01 | WhatsApp-Bridge: **Gateway-nativ gemanaged** (config.yaml `whatsapp: enabled, mode: bridge`; Gateway startet `bridge.js --mode bot` als Kindprozess) — **Pairing ✅ 10:40**, /health `{"status":"connected"}`, scriptHash 6830e1f5. **Architektur-Fix 14:10:** separater systemd-Service `nexifyai-whatsapp-bridge.service` (Baileys-Altlast, self-chat) gestoppt+disabled — er erzeugte Port-Konflikt/Restart-Loop; NUR Gateway-Spawn auf 3000. **Live-Bot-Modus aktiv** → WA-TEST-01 (t_b4bbbd39, needs_input Pascal): 4 Testnachrichten; danach M-06 (t_6e059036) schließen. Ban-Risiko Baileys dokumentiert → Cloud-API-Strategie WA-API-01 (Pricing-Recherche 08.08.: Per-Message ab 07/2025, AI-Provider seit 13.05.2026 ohne Non-Template-Gebühr DE/NL, Service-Message-Gebühr ab 01.10.2026 = Kostenrisiko Dialog). | ⏳ WA-TEST-01 → M-06 |
...
| Datum | Änderung |
|---|---|
| 2026-08-08 15:35 | **M-09 CTA-/SNIPPET-OPTIMIERUNG + LINK-VALIDIERUNG (t_d3a3f169, P1):** (1) **Titel-Audit systemweit:** 56 metadata-Dateien geprueft; Stadtseiten (M-03) mit Frewert-Muster "[Kernleistung] [Ort] » Gratis KI-Audit" = korrekt; Leistungs-/Branchenseiten nutzen Keyword-Intent-Titel (M-02/M-03-Referenz) — keine Umformulierung ohne SEO-Begruendung (AUSSCHLUSS). (2) **Description-Audit:** alle Marketing-Seiten mit CTA-Verben + Nutzenversprechen; Rechtstexte bewusst nuechtern (Conversion-frei). (3) **UTM-Vollstaendigkeit (Trick 23):** utm_source/medium/campaign an Konversions-CTAs — Stadt-/Branchen-/Leistungs-/Preise-Seiten + Rechner + Wissen-Artikel (19 Vorkommen, utm_medium=organic nachgezogen: stadt/branchen/leistungen/preise-pakete). Header/Footer/Sticky/ExitIntent ohne utm_medium = bewusst (Kampagnen-Attribution nur auf Conversion-Pfaden; interne Navigations-Links sind keine Kampagnen). (4) **Link-Validierung (§14.9):** 67 URLs (Header/Footer/Sitemap/Dynamik) gegen Live-Site — 0 tote Links; Stadtseiten fehlten in Sitemap → ergaenzt (10 URLs, live 200). (5) **Schema-Haertung (M-02-Konvergenz):** partner/botschafter BreadcrumbList-JSON-LD ergaenzt, Homepage FAQPage aus home-faqs.ts, Organization-@id/ContactPoint/Logo im Root-Layout — Tests rot→gruen. **Qualitaetsgates:** 222/222 Tests, typecheck ok, build 175/175, lint 0 errors; Vercel-Deploy success (3 Commits c986fdbc/b8f6c207/df6f8405, LIVE verifiziert: Titel + UTM im HTML). GitLab-Branch m09-cta-snippet (main protected). **GEGENTEST BESTANDEN:** Negativfall (URL ohne Titel-Muster → nur /wissen-Artikel, bewusst kein Gratis-Anreiz), Regression 222/222. Scripts: scripts/m09-snippet-audit.sh + m09-linkcheck.sh (wiederverwendbar). |
...
| 2026-08-08 15:10 | **DISPATCHER-PARALLELITÄT (CEO-Runde 5):** Root-Cause Board-Stau: (1) Gateway-Dispatcher nach Neustart stuck („ready queue non-empty … 0 workers spawned"), (2) CLI/Daemon sahen Board **default** statt nexify (current-Symlink wird nicht aufgelöst → HERMES_KANBAN_BOARD=nexify env nötig), (3) nur 1 Worker/Profil (nur 3 Profile). **Fix:** eigenständiger **nexifyai-kanban-daemon.service** (systemd, `hermes kanban daemon --force --interval 60 --max 3`, HERMES_KANBAN_BOARD=nexify, Log /var/log/nexifyai/kanban-daemon.log); `kanban.dispatch_in_gateway=false` gesetzt (greift beim nächsten Gateway-Restart). **Neue Worker-Profile** marketing-a + marketing-b (DeepSeek v4-flash via 9Router, Think-Max, max_turns 80→**250** — 80er-Budget war zu klein: M-02 `Iteration budget exhausted (80/80)`). 6 M-Tasks umgehängt (M-02/03/04 → marketing-a; M-05/07/08 → marketing-b). **Ergebnis: 3 Worker parallel** (default: M-01; marketing-a: M-03+M-04). SEC-02 (Pascal-only) in **todo** statt blocked (Dispatchers promoted blocked→ready wiederholt; todo wird nie dispatched). MARKET-02 (UWG) done. |
| 2026-08-08 14:20 | **SYSTEMWEITE HÄRTUNG (CEO-Runde 3):** (1) **Worker-Crash-Loop behoben:** hermes-webui-Container 4GiB→**8GiB** Memory-Limit (docker update live) — Worker starben alle ~16min (Cgroup-OOM, `pid not alive`); M-01 lief danach 3h stabil. (2) **M-01 Root-Cause:** `Iteration budget exhausted (500/500)` → **goal_mode=1 (40 Turns) für 11 große M-Tasks** aktiviert (M-01..M-11, M-13). (3) **WhatsApp-Bridge:** Gateway-nativ verifiziert (`--mode bot` Kindprozess des Gateways); redundanter systemd-Service gestoppt+disabled (Port-Konflikt/Restart-Loop eliminiert). (4) **Website-Chat-Härtung (E2E+Gegentest bestanden):** SYSTEM_PROMPT um KI-Offenlegung (Art. 50, Erstkontakt „Ich bin der KI-gestützte Berater…") + CONFIDENTIALITY-GUARD (keine Interna/Secrets/Kundendaten Dritter; Verweis auf persönlichen Kontakt) erweitert — Commit f19ea1eb, Backend live. (5) **GEO/LLM-SEO-Recherche 2026** (E3) → Bericht cron/output + M-13-Umsetzungs-Task. (6) **SEC-02** erneut blocked (Dispatcher-Promote verhindert). |
| 2026-08-08 13:10 | **P0-SECURITY: SECRET-LEAK GESCHLOSSEN (public GitHub-Repo):** `ARBEITSVORGABEN-v3.3.md` (Commit c3b22291) enthielt kompletten Zugangsdaten-Block (GitHub-PATs, Cloudflare, Vercel, Supabase-Cloud, DB-PW) — Leak ~24h public. **Behoben:** Datei redigiert (*** + Verweis hermes.env, Commit 6874ccab), git-filter-repo über ALLE Branches (1048+ Commits), force-push origin+gitlab (main + 110 Branches, GitLab-Branch temporär unprotectet→wieder protected push_level 0), Final-Scan 0 Funde. verify-vorgaben-sync.py auf v3.3 aktualisiert → GEGENTEST BESTANDEN. WebUI-SOUL MD5-sync. **Offen (Task SEC-02 t_4e7b135d, blocked needs_input, nur Pascal):** GitHub-PATs + Vercel-Token waren zum Leak-Zeitpunkt GÜLTIG (HTTP 200) → Rotation im Dashboard nötig; Cloudflare-3er-Set bereits INVALID (kein aktives Skript betroffen). |
| 2026-08-08 10:55 | **MAIL-02-DEPLOY (t_86c628f4, System-CEO-Auflagen A/B):** M2/M3-Templates (`lead_email_followup_m2.html`, `lead_email_breakup_m3.html`) in Produktion (`/usr/local/share/nexifyai-templates/`, Opt-out-Link `/api/outreach/unsubscribe?email&token` ✅, kein alter Footer). `drip-campaign.py` (3 Kopien synchron: `/usr/local/bin/nexifyai-drip-campaign.py`, `/workspace/scripts/drip-campaign.py`, Monorepo `infra/lead-pipeline/scripts/`) auf KOMM-001 v2.0: M2→`lead_email_followup_m2.html` Tag 4 Betreff „kurze Nachfrage zu meiner Mail", M3→`lead_email_breakup_m3.html` Tag 10 Betreff „letzte Nachricht"; Auflage A `--dry-run`-Flag (kein Versand/State-Write, E2E bestanden: 10 DRY-RUN-Zeilen, State-MD5 unverändert); Auflage B Status-Break `('contacted','opted_out','unsubscribed')` + `unsubscribed`-Feld (Art. 21). Backup `/root/.local-backups/mail02-20260808/`. Timer bleibt aktiv (täglich 10:00, IST — keine Änderung). Doku: KOMM-001-Referenz in Docstring. |
...
| 2026-08-08 14:35 | **M-11 CAMOUFOX-DOCKER LIVE (Anti-Bot-Fallback, P2):** Container `camofox-browser` (ghcr.io/jo-inc/camofox-browser, Port 9377->9377, Restart=no) laeuft auf Host srv1243952 via SSH-Loopback 2222. Health http://127.0.0.1:9377/health = 200 (engine=camoufox, browserConnected=true, activeTabs=1). E2E bestanden (camofox_e2e_test.py): Routing-Logik korrekt (is_camofox_mode True nur ohne CDP-Override — Produktion bleibt Chrome-CDP 9222), navigate+snapshot auf example.com ok (237 chars, "Example Domain"), Negativtest Backend-down -> saubere Fehlermeldung "Cannot connect to Camofox", kein Crash. CAMOFOX_URL=http://127.0.0.1:9377 in /root/.hermes/.env gesetzt. Einsatz nur bei Bot-Blockern; keine Profil-Daten-Kopie in andere Systeme. |
