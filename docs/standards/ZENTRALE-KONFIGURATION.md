...
| Befugnis | ADMIN-Vollmacht: A4-Aktionen (DB-Schema, SSH-Härtung, Deploys) eigenverantwortlich; 1-Profil-Baseline (nur `default`, 19→1 konsolidiert 2026-08-07, Rollen via nexifyai-*-Skills) |
...
| Ebene | Profil (1-Profil-Baseline) |
|---|---|
| Steuerung | `default` (System-CEO; Rollen via nexifyai-*-Skills: architekt, developer, devops, reviewer, marketer, support, task-execution …) |
| Squads | Konzept entfällt — 19→1-Konsolidierung 2026-08-07 (nur `default`); Spezialisierung über Skills, nicht Profile |
...
| TDDDG-01 | Cookie-Banner E2E-Test ausstehend (Browser-Daemon down) | ✅ (2026-08-07 via AUDIT-01: Chunk 36jvr0kg-fy-o.js, nexifyai-cookie-consent localStorage, Nur-Essenzielle+Alle-akzeptieren live) |
| AGENTS-02 | 3-Ebenen-Org finalisieren (Design-/Security-Squad) | ✅ (abgelöst durch 19→1-Profil-Baseline; Rollen via Skills) |
...
| Datum | Änderung |
|---|---|
| 2026-08-07 | **AUDIT-01 Altdaten-Bereinigung + Verbindungs-Audit (19→1-Baseline)**: (1) Board: 24 Tasks auf `default` umgehängt, 6 stale Tasks geschlossen (SEC-01/AGENTS-01 ZK-belegt, TDDDG-01 live verifiziert, SEC-03 RestartPolicy OK, AGENTS-02 durch 1-Profil-Baseline abgelöst, t_be82fc43 LeadGen 35/36 bestanden); (2) LightRAG-Embedding-Fix P0: `EMBEDDING_API_KEY`→`EMBEDDING_BINDING_API_KEY` (DeepSeek-only-Direktive hatte OPENAI_API_KEY auf 9Router gestellt → 401 an Upstage; E2E PROCESSED, Smoke pass=10 fail=0, Backup .bak-20260807-audit01); (3) toter Container hermes-webui-old-backup entfernt (Smoke-Fail-Ursache); (4) Doku-Drift §9/§11 bereinigt (19-Profile-Org→1-Profil, TDDDG-01 ✅, AGENTS-02 ✅); (5) 7/7 Domains + 16 Ports live validiert; hermes.env 285 Keys, keine Cloud-Supabase-Refs; OPS-04 mdlgodcv-Altlast nur noch in credentials.env (kein hermes.env) |
