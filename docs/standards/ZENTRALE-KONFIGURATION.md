...
> **Kanonische Vorgaben:** `docs/standards/ARBEITSVORGABEN-v3.3.md` (§0–§14, Pascal-Direktive — Abweichungs-Null-Toleranz, Betriebshandbuch-Pflicht, Online-Recherchepflicht).
...
| Befugnis | ADMIN-Vollmacht: A4-Aktionen (DB-Schema, SSH-Härtung, Deploys) eigenverantwortlich; Sub-Agenten-Team (Konsolidierung 19→1, 2026-08-07: nur `default` aktiv) |
...
Kanonische Fassung: `docs/standards/ARBEITSVORGABEN-v3.3.md` (Quelle: SOUL.md v3.3, System-Prompt).
...
| Datum | Änderung |
|---|---|
| 2026-08-07 20:25 | **BULK-SMTP-TIMEOUT-FIX (P1, live):** Bulk-Lauf hing 19:46–19:51 (25 Min ohne Fortschritt bei [RE] 26/102, Prozess in hrtimer_nanosleep, keine SMTP-Verbindung). Root-Cause: `src/pipeline/email_lead.py` `smtplib.SMTP_SSL(...)` OHNE `timeout=` → stiller SMTP-Server blockiert endlos; die 30s-Retry-Logik des Aufrufers greift nie. Fix: `timeout=60` + `socket.setdefaulttimeout(60)` (Backup `.bak-20260807-*`), py_compile OK, E2E-Gegentest negativ (toter Port → False in 0.0s statt Hänger). Deploy: `/workspace/src/pipeline/email_lead.py` + Repo-Spiegel `infra/lead-pipeline/src/phases/outreach.py` (MD5 identisch d7e0d502), Commit a959d858. Neustart: State-Guard `reengagement[]` verhinderte Doppelversand — Lauf setzte nahtlos bei 75 Rest-Kandidaten fort (PID 2465181, aktiv, 0 Fehler). Offen: Log-Stall-Watchdog (>10 Min ohne Logzeile → Alarm) als Task angelegt (fehlende Automatisierung, CEO-Mandat 6). |
| 2026-08-07 20:05 | **DOKU-SYNC-ROUNDE (Datei-für-Datei-Audit der verbindlichen Dokumente):** (1) `docs/standards/ARBEITSVORGABEN-v3.3.md` NEU angelegt (= `/home/hermeswebui/.hermes/SOUL.md` v3.3, MD5 ed007533, Artefakt-Zeile ```markdown entfernt) — vorher fehlte die Datei trotz überall referenzierter v3.3; (2) Dateinamen-Referenzen vereinheitlicht v2.2/v2.3 → v3.3.md: `/root/.hermes/SOUL.md`, `/workspace/nexifyai/hermes/AGENTS.md`, ZK §1a + §12; (3) CHARTA.md §10 korrigiert (Altstand „Ersatz durch Upstage" → DeepSeek-only-Ergänzung 2026-08-07); (4) USER.md Altdaten gefixt (Workspace-Pfad /opt/nexifyai/workspace → /workspace/nexifyai + Repo; Beispiel-Modell tencent/hy3-preview → deepseek-v4-flash-0731); (5) Geprüft ohne Änderung: IDENTITY.md (aktuell), CEO-MISSION-2026-08-07.md (aktuell), MEMORY.md, /root/.hermes/AGENTS.md (Symlink intakt). Notiert für AUDIT-01: CLAUDE.md „Aktuelles Status (2026-07-24)"-Block + Architektur-Diagramm (vitrine/konsole/Traefik) = Altdaten; ZK §9 Sub-Agenten-Tabelle (17+2) = Vor-Konsolidierungsstand. |
| 2026-08-07 19:45 | **BULK-18:25-CRASH GEFIXT (P0) + DASHBOARD-PROFIL-SYNC (Pascal-Befund „Kanäle nutzen uraltes Wissen"):** (1) nexifyai-bulk-send 18:25 crashte im Restart-Loop (`Template laden fehlgeschlagen: templates/lead_email_followup.html`, Errno 2) — v2.2 lädt 4 Templates relativ zu /workspace, Deploy hatte nur 3 → `lead_email_followup.html` + `_m2` aus /usr/local/share nach /workspace/templates deployt (MD5-identisch) → Auto-Restart startete Lauf (~18:33, Welle [RE] 102 Kandidaten, 0 Fehler, PATCH 200); Repo-Spiegel `infra/lead-pipeline/templates/` um m2/m3 ergänzt (Commit 8bb430a1, origin→SSH); Skill nexify-mailing-pipeline Pitfall „Template-Deploy VOLLSTÄNDIG" ergänzt (staged). (2) **Dashboard-Chat (hermes-dash.nexifyai.cloud/chat) lief mit `--open-profile inbox-triage` = uraltes Profil (SOUL.md 28.07, profiles.old)** → Unit auf `--open-profile default` umgestellt (sed+daemon-reload+restart), verifiziert (ps E2, 9119→302, /api/status: Telegram connected, Hermes 0.20.0). Gateway (18:09, default) war bereits aktuell. (3) Dispatcher-Lesson: 4 Worker-Tasks crashed 15:31–15:48 (`pid not alive`, Dashboard-Neustart 16:00 killte Worker-Kinder) → unblockt (AUDIT-01/SEC-04/MAIL-03/WEB-01), laufen wieder. AgentMemory: mem_msj4yla7 (Dispatcher), mem_msj7wpj6 (Bulk), mem_msj9… (Dashboard). |
