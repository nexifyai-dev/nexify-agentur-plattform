...
5. **DeepSeek-only (2026-08-07):** Systemweit NUR `ds/deepseek-v4-flash` (Standard, Think-Max) und `ds/deepseek-v4-pro` (nur echte Komplexität). **KEIN OpenRouter; LLM ausschließlich DeepSeek via 9Router. Embedding: NUR via Upstage (OpenAI-kompatibel `/v1/embeddings` auf `api.upstage.ai`, Modell `solar-embedding-1-large-passage`/`-query` bzw. Alias `embedding-passage`/`embedding-query`, `UPSTAGE_API_KEY` — OHNE Suffix invalid, E3 2026-08-13; Pascal 2026-08-13, vollintegriert lt. API-Doku); Upstage-Document-AI wo passend; lokale FTS5-Kurzpfade bleiben.** Kein Modell-Call ohne 9Router (`http://127.0.0.1:20128/v1`).
