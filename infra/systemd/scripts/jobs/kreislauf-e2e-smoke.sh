...
if [ "$(secret_available)" != "true" ]; then
  ...
  TRACK_ID="$(curl -s -m 15 -H 'Content-Type: application/json' \
    -X POST http://127.0.0.1:9622/documents/text \
    --data "{\"text\":\"${SYNC_TEXT}\",\"file_source\":\"${SYNC_PREFIX}\"}" \
    | python3 -c 'import json,sys; d=json.load(sys.stdin); print(d.get("track_id",""))' 2>/dev/null)"
  ...
  if [ -z "$TRACK_ID" ]; then
    ...
    for i in $(seq 1 40); do
      SEEN="$(curl -s -m 10 "http://127.0.0.1:9622/documents/track_status/${TRACK_ID}" \
    ...
    if [ "$SEEN_UP" != "PROCESSED" ]; then
      ...
      SYNC_FILE_PATH="$(curl -s -m 10 "http://127.0.0.1:9622/documents/track_status/${TRACK_ID}" \
