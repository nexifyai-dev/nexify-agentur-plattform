...
def slot_dict(r) -> dict:
    return {
        "id": str(r["id"]), "start_at": r["start_at"].isoformat(), "duration_min": r["duration_min"],
        "status": r["status"], "name": r["name"], "email": r["email"], "phone": r["phone"],
        "company": r["company"], "topic": r["topic"], "language": r["language"],
    ...
    async with pool.acquire() as con:
        for s in slots:
            try:
                ...
                skipped += 1
                continue
            exists = await con.fetchval("select 1 from nexify_slots where start_at = $1", dt)
            if exists:
                skipped += 1
                continue
            await con.execute(
                "insert into nexify_slots (id,start_at,duration_min) values ($1,$2,$3)",
                uuid.uuid4(), dt, duration_min,
            )
            created += 1
...
async def delete_slot_core(slot_id: str) -> dict:
    pool = await _DB()
    async with pool.acquire() as con:
        res = await con.execute("delete from nexify_slots where id = $1 and status = 'free'", uuid.UUID(slot_id))
...
async def public_slots():
    ...
    return [{"id": s["id"], "start_at": s["start_at"], "duration_min": s["duration_min"]} for s in slots]
...
async def book_slot(body: BookIn):
    ...
    async with pool.acquire() as con:
        row = await con.fetchrow(
            ...
            uuid.UUID(body.slot_id), body.name, body.email.lower(), body.phone, body.company, body.topic, body.language,
        )
        if not row:
            raise HTTPException(status_code=409, detail="Dieser Termin ist leider nicht mehr verfügbar. Bitte wählen Sie einen anderen." if body.language != "nl" else "Deze afspraak is helaas niet meer beschikbaar. Kies een ander tijdstip.")
        await con.execute(
            "insert into nexify_leads (id,name,email,company,phone,language,message,source) values ($1,$2,$3,$4,$5,$6,$7,'callback')",
            uuid.uuid4(), body.name, body.email.lower(), body.company, body.phone, body.language,
            f"Rückruf-Termin gebucht: {fmt_slot(row['start_at'])}" + (f" – Thema: {body.topic}" if body.topic else ""),
        )
    nl = body.language == "nl"
    when = fmt_slot(row["start_at"], body.language)
    title = "Uw terugbelafspraak is bevestigd" if nl else "Ihr Rückruf-Termin ist bestätigt"
    body_html = (
        ...
        if nl else
        f"Guten Tag {body.name},<br/><br/>vielen Dank – Ihr Rückruf-Termin ist verbindlich gebucht:<br/><br/>"
    ...
    asyncio.create_task(_SEND_EMAIL(body.email, f"{title} – {when}", _CI_EMAIL(title, body_html, language=body.language)))
    ...
    return {"status": "booked", "start_at": row["start_at"].isoformat(), "formatted": when}
