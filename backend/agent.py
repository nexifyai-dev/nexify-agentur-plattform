...
6. PREISE & ZUSAGEN: Tagessatz 999 EUR netto. Keine Rabatte, Fristen- oder Rechtszusagen ohne explizite Admin-Freigabe.
...
TOOLS = [
    _tool("get_stats", "Kennzahlen: Leads, Angebote, Kunden, Pipeline, gewonnener Umsatz.", {}),
    _tool("search_leads", "Leads/Interessenten durchsuchen (Name, E-Mail, Firma, Telefon, Nachricht). Leerer query = neueste zuerst.", {"query": S, "limit": N}),
    _tool("update_lead", "Lead-Daten ändern. Nur übergebene Felder werden aktualisiert. status: new|contacted|qualified|won|lost.", {"lead_id": S, "name": S, "email": S, "company": S, "phone": S, "status": S, "message": S}, ["lead_id"]),
    _tool("create_prospect", "Interessenten manuell anlegen: erzeugt Lead + sendet Willkommens-E-Mail mit Portal-Einladung. language: de|nl.", {"name": S, "email": S, "company": S, "phone": S, "language": S, "note": S}, ["name", "email"]),
    _tool("list_offers", "Angebote auflisten. status optional: sent|followed_up|accepted|declined.", {"status": S, "limit": N}),
    _tool("get_offer", "Angebot inkl. Positionen und Nachrichtenverlauf abrufen.", {"offer_id": S}, ["offer_id"]),
    _tool("create_and_send_offer", "Angebot erstellen und per E-Mail (CI-Design + PDF) versenden, inkl. Portal-Einladung. items: [{name, description, days_min, days_max}]. Preis = Tage x 999 EUR.", {"name": S, "email": S, "company": S, "language": S, "title": S, "intro": S, "items": {"type": "array", "items": {"type": "object", "properties": {"name": S, "description": S, "days_min": N, "days_max": N}, "required": ["name"]}}, "assumptions": {"type": "array", "items": S}, "next_steps": {"type": "array", "items": S}}, ["name", "email", "title", "items"]),
    _tool("reply_offer", "Antwort auf Angebots-Rückfrage: erscheint im Kundenportal UND wird per E-Mail gesendet.", {"offer_id": S, "body": S}, ["offer_id", "body"]),
    _tool("list_tickets", "Support-Tickets auflisten. status optional: open|answered|closed.", {"status": S, "limit": N}),
    _tool("get_ticket", "Ticket inkl. komplettem Nachrichtenverlauf abrufen.", {"ticket_id": S}, ["ticket_id"]),
    _tool("reply_ticket", "Ticket beantworten: erscheint im Portal UND wird per E-Mail gesendet. body = reiner Text.", {"ticket_id": S, "body": S}, ["ticket_id", "body"]),
    _tool("send_email", "Individuelle E-Mail im NeXify-CI-Design senden (Absender mail@nexifyai.cloud). body = reiner Text, Absätze durch Leerzeilen.", {"to": S, "subject": S, "body": S, "language": S}, ["to", "subject", "body"]),
    _tool("list_slots", "Alle Rückruf-Zeitfenster (frei + gebucht) der nächsten Zeit inkl. Kontaktdaten der Bucher.", {}),
    _tool("create_slots", "Neue freie Rückruf-Zeitfenster anlegen. slots = Liste ISO-Zeitpunkte Europe/Amsterdam, z. B. ['2026-06-05T14:30']. duration_min Standard 30.", {"slots": {"type": "array", "items": S}, "duration_min": N}, ["slots"]),
    _tool("delete_slot", "Freies (ungebuchtes) Zeitfenster löschen.", {"slot_id": S}, ["slot_id"]),
    _tool("schedule_task", "Zeitgesteuerten Folge-Task für dich selbst planen. run_at = ISO-Zeitpunkt (Europe/Amsterdam). instruction = präzise, in sich geschlossene Anweisung inkl. aller IDs/E-Mail-Adressen.", {"run_at": S, "title": S, "instruction": S}, ["run_at", "title", "instruction"]),
    ...
    try:
        if name == "get_stats":
            async with pool.acquire() as con:
                ...
                customers = await con.fetchval("select count(*) from nexify_users where role='customer'")
                ...
                open_tickets = await con.fetchval("select count(*) from nexify_tickets where status='open'")
                booked = await con.fetchval("select count(*) from nexify_slots where status='booked' and start_at > now()")
            return {"leads": leads, "offers": offers, "accepted": accepted, "customers": customers,
                    "pipeline_eur": float(pipeline), "won_eur": float(won), "open_tickets": open_tickets, "upcoming_calls": booked}
        ...
        if name == "search_leads":
            ...
            return {"leads": [{"id": str(r["id"]), "name": r["name"], "email": r["email"], "company": r["company"],
                               "phone": r["phone"], "language": r["language"], "status": r["status"], "source": r["source"],
                               "message": (r["message"] or "")[:300], "created_at": r["created_at"].isoformat()} for r in rows]}
        ...
        if name == "create_prospect":
            return await portal.prospect_core(args["name"], args["email"], args.get("company"),
                                              args.get("phone"), args.get("language", "de"), args.get("note"))

        if name == "list_offers":
            limit = int(args.get("limit") or 20)
            async with pool.acquire() as con:
                if args.get("status"):
                    rows = await con.fetch("select * from nexify_offers where status=$1 order by created_at desc limit $2", args["status"], limit)
                else:
                    rows = await con.fetch("select * from nexify_offers order by created_at desc limit $1", limit)
            return {"offers": [{"id": str(r["id"]), "name": r["name"], "email": r["email"], "status": r["status"],
                                "payment_status": r["payment_status"], "price_total": float(r["price_total"] or 0),
                                "title": (json.loads(r["offer_json"]) if r["offer_json"] else {}).get("title", ""),
                                "created_at": r["created_at"].isoformat()} for r in rows]}

        if name == "get_offer":
            async with pool.acquire() as con:
                r = await con.fetchrow("select * from nexify_offers where id=$1", uuid.UUID(args["offer_id"]))
                if not r:
                    return {"error": "Angebot nicht gefunden"}
                msgs = await con.fetch("select * from nexify_offer_messages where offer_id=$1 order by created_at asc", uuid.UUID(args["offer_id"]))
            return {"id": str(r["id"]), "name": r["name"], "email": r["email"], "company": r["company"],
                    "language": r["language"], "status": r["status"], "payment_status": r["payment_status"],
                    "price_total": float(r["price_total"] or 0), "offer": json.loads(r["offer_json"]) if r["offer_json"] else None,
                    "created_at": r["created_at"].isoformat(),
                    "messages": [{"sender": m["sender"], "body": m["body"], "at": m["created_at"].isoformat()} for m in msgs]}

        if name == "create_and_send_offer":
            return await portal.offer_core(
                args["name"], args["email"], args.get("company"), args.get("language", "de"),
                args["title"], args.get("intro", ""),
                [{"name": i.get("name", ""), "description": i.get("description", ""),
                  "days_min": float(i.get("days_min", 1)), "days_max": float(i.get("days_max", i.get("days_min", 1)))} for i in args["items"]],
                args.get("assumptions", []), args.get("next_steps", []))
        ...
        if name == "list_tickets":
            ...
            async with pool.acquire() as con:
                if args.get("status"):
                    rows = await con.fetch("select * from nexify_tickets where status=$1 order by created_at desc limit $2", args["status"], limit)
                else:
                    rows = await con.fetch("select * from nexify_tickets order by created_at desc limit $1", limit)
            return {"tickets": [{"id": str(r["id"]), "name": r["name"], "email": r["email"], "subject": r["subject"],
                                 "status": r["status"], "source": r["source"], "created_at": r["created_at"].isoformat()} for r in rows]}

        if name == "get_ticket":
            async with pool.acquire() as con:
                r = await con.fetchrow("select * from nexify_tickets where id=$1", uuid.UUID(args["ticket_id"]))
                if not r:
                    return {"error": "Ticket nicht gefunden"}
                msgs = await con.fetch("select * from nexify_ticket_messages where ticket_id=$1 order by created_at asc", uuid.UUID(args["ticket_id"]))
            return {"id": str(r["id"]), "name": r["name"], "email": r["email"], "subject": r["subject"],
                    "status": r["status"], "language": r["language"], "source": r["source"],
                    "messages": [{"sender": m["sender"], "body": m["body"], "at": m["created_at"].isoformat()} for m in msgs]}
        ...
        if name == "send_email":
            ...
            html_body = "".join(f"<p>{p.replace(chr(10), '<br/>')}</p>" for p in args["body"].split("\n\n") if p.strip())
            email_id = await _SEND_EMAIL(args["to"], args["subject"], _CI_EMAIL(args["subject"], html_body, language=lang))
        ...
        if name == "create_slots":
            return await booking.create_slots_core(args["slots"], int(args.get("duration_min") or 30))
        ...
        if name == "schedule_task":
            ...
            tid = uuid.uuid4()
            async with pool.acquire() as con:
                await con.execute("insert into nexify_agent_tasks (id,title,instruction,run_at) values ($1,$2,$3,$4)",
                                  tid, args["title"], args["instruction"], run_at)
            return {"status": "scheduled", "task_id": str(tid), "run_at": run_at.isoformat()}

        if name == "list_tasks":
            async with pool.acquire() as con:
                rows = await con.fetch("select * from nexify_agent_tasks order by run_at desc limit 50")
            return {"tasks": [{"id": str(r["id"]), "title": r["title"], "instruction": r["instruction"][:300],
                               "run_at": r["run_at"].isoformat(), "status": r["status"],
                               "result": (r["result"] or "")[:300]} for r in rows]}

        if name == "cancel_task":
            async with pool.acquire() as con:
                res = await con.execute("update nexify_agent_tasks set status='cancelled' where id=$1 and status='pending'", uuid.UUID(args["task_id"]))
            ...
            async with pool.acquire() as con:
                rows = await con.fetch(
                    """select s.id, s.language, s.created_at, count(m.id) as n, max(m.created_at) as last_at
                       ...
                       group by s.id order by coalesce(max(m.created_at), s.created_at) desc limit $1""", limit)
            return {"sessions": [{"id": str(r["id"]), "language": r["language"], "messages": r["n"],
                                  "last_at": (r["last_at"] or r["created_at"]).isoformat()} for r in rows]}
...
async def _save_agent_msg(role: str, content: str):
    pool = await _DB()
    async with pool.acquire() as con:
        await con.execute("insert into nexify_agent_messages (id,role,content) values ($1,$2,$3)", uuid.uuid4(), role, content)


async def _load_history(limit: int = 30) -> list[dict]:
    pool = await _DB()
    async with pool.acquire() as con:
        rows = await con.fetch("select * from nexify_agent_messages order by created_at desc limit $1", limit)
    out = []
    for r in reversed(rows):
        if r["role"] == "action":
            try:
                a = json.loads(r["content"])
                out.append({"role": "assistant", "content": f"[Ausgeführte Aktion: {a.get('tool')} → {a.get('summary', '')}]"})
            except Exception:
                pass
...
async def run_agent(user_message: str, autonomous: bool = False, persist: bool = True, max_rounds: int = 8):
    ...
    for _ in range(max_rounds):
        resp = await _LLM.chat.completions.create(model=_MODEL, messages=messages, tools=TOOLS, max_tokens=4000)
        ...
        if msg.tool_calls:
            ...
            for tc in msg.tool_calls:
                ...
                if persist:
                    await _save_agent_msg("action", json.dumps(action, ensure_ascii=False))
...
async def agent_history(_: dict = Depends(portal.get_admin)):
    pool = await _DB()
    async with pool.acquire() as con:
        rows = await con.fetch("select * from nexify_agent_messages order by created_at desc limit 120")
    return [{"id": str(r["id"]), "role": r["role"], "content": r["content"], "created_at": r["created_at"].isoformat()} for r in reversed(rows)]
...
async def agent_tasks(_: dict = Depends(portal.get_admin)):
    pool = await _DB()
    async with pool.acquire() as con:
        rows = await con.fetch("select * from nexify_agent_tasks order by run_at desc limit 100")
    return [{"id": str(r["id"]), "title": r["title"], "instruction": r["instruction"], "run_at": r["run_at"].isoformat(),
             "status": r["status"], "result": r["result"], "created_at": r["created_at"].isoformat()} for r in rows]
...
async def task_worker():
    while True:
        try:
            ...
            if pool:
                async with pool.acquire() as con:
                    rows = await con.fetch("select * from nexify_agent_tasks where status='pending' and run_at <= now() limit 3")
                for row in rows:
                    async with pool.acquire() as con:
                        claimed = await con.execute("update nexify_agent_tasks set status='running' where id=$1 and status='pending'", row["id"])
                    if not claimed.endswith("1"):
                        continue
                    logger.info(f"agent task starting: {row['title']}")
                    try:
                        result = await run_agent(
                            f"[GEPLANTER TASK „{row['title']}“ – jetzt fällig]\n{row['instruction']}", autonomous=True)
                        ...
                        status = "failed"
                    async with pool.acquire() as con:
                        await con.execute("update nexify_agent_tasks set status=$1, result=$2 where id=$3", status, summary, row["id"])
                    notify = os.environ.get("INTERNAL_NOTIFY_EMAIL")
                    if notify:
                        await _SEND_EMAIL(notify, f"Agent-Task erledigt: {row['title']}",
                                          _CI_EMAIL(f"Agent-Task: {row['title']}",
                                                    f"<p>Status: <b>{status}</b></p><p style='border-left:2px solid #52525b;padding-left:12px;'>{summary.replace(chr(10), '<br/>')}</p>",
                                                    cta_label="Zum Agent-Cockpit", cta_url=f"{FRONTEND_URL}/admin"))
        except Exception as e:
