...
function Toggle({ on, disabled, onChange, testId }: { on: boolean; disabled?: boolean; onChange?: (v: boolean) => void; testId: string }) {
  return (
    <button
      ...
      onClick={() => onChange?.(!on)}
      data-testid={testId}
      className={`relative h-7 w-11 shrink-0 rounded-full transition-colors ${on ? "bg-emerald-400/80" : "bg-white/15"} ${disabled ? "cursor-not-allowed opacity-60" : ""}`}
      aria-pressed={on}
    >
      <span className={`absolute top-0.5 size-6 rounded-full bg-white transition-transform ${on ? "translate-x-[18px]" : "translate-x-0.5"}`} />
    </button>
  );
}
...
export function CookieConsent() {
  ...
  return (
    <div
      className="pointer-events-none fixed inset-x-0 bottom-0 z-[90] p-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] sm:p-6"
    ...
    >
      <div className="pointer-events-auto mx-auto mb-[4.5rem] max-w-2xl rounded-2xl border border-white/15 bg-black/90 p-4 shadow-[0_20px_60px_rgba(0,0,0,0.6)] backdrop-blur-xl sm:mb-0 sm:p-6">
          ...
          <div className="mt-4 max-h-[40vh] space-y-3 overflow-y-auto border-t border-white/10 pt-4 sm:max-h-none" data-testid="cookie-settings-panel">
            {[
              ...
              <div key={c.testId} className="flex items-start justify-between gap-4">
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2 text-[13px] font-semibold text-zinc-200">
                    {c.label}
        ...
        <div className="mt-4 flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center">
          <button className="btn-primary min-h-11 w-full !px-5 !py-2.5 !text-[13px] sm:w-auto" onClick={() => persist(true, true)} data-testid="cookie-accept-all">
            {t.acceptAll}
          </button>
          <button className="btn-ghost min-h-11 w-full !px-5 !py-2.5 !text-[13px] sm:w-auto" onClick={() => persist(false, false)} data-testid="cookie-necessary-only">
            {t.necessaryOnly}
          </button>
          {detail ? (
            <button className="btn-ghost min-h-11 w-full !px-5 !py-2.5 !text-[13px] sm:w-auto" onClick={() => persist(analytics, marketing)} data-testid="cookie-save">
              {t.save}
            </button>
          ) : (
            <button className="inline-flex min-h-11 w-full items-center justify-center gap-1.5 px-3 py-2 text-[13px] font-semibold text-zinc-400 transition-colors hover:text-white sm:w-auto" onClick={() => setDetail(true)} data-testid="cookie-open-settings">
