...
code=$(curl -sS -o /tmp/gl-proj.json -w "%{http_code}" --max-time 8 "$API_URL/projects?per_page=1" || echo 000)
if [[ "$code" == "200" ]]; then
...
if [[ -z "${GITLAB_PERSONAL_ACCESS_TOKEN:-${GITLAB_TOKEN:-}}" ]]; then
  ...
  code=$(curl -sS -o /tmp/gl-user.json -w "%{http_code}" --max-time 8 \
    -H "PRIVATE-TOKEN: $TOKEN" "$API_URL/user" || echo 000)
  if [[ "$code" == "200" ]]; then
    user=$(python3 -c "import json;print(json.load(open('/tmp/gl-user.json')).get('username','?'))" 2>/dev/null || echo '?')
    ok "GET /user → 200 (username=$user)"
  else
    bad "GET /user → $code (Token ungültig oder Scope fehlt)"
  fi

  # Project search (group nexifyai or nexify)
  for path in "nexifyai/nexify-agentur-plattform" "nexify/nexify-agentur-plattform"; do
    enc=$(python3 -c "import urllib.parse;print(urllib.parse.quote('$path',safe=''))")
    code=$(curl -sS -o /tmp/gl-p.json -w "%{http_code}" --max-time 8 \
      -H "PRIVATE-TOKEN: $TOKEN" "$API_URL/projects/$enc" || echo 000)
    if [[ "$code" == "200" ]]; then
      ok "Projekt gefunden: $path"
      break
    else
      warn "Projekt nicht gefunden: $path ($code)"
    fi
  done
fi
