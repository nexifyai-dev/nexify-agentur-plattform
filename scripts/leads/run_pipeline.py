...
# NAME: NeXifyAI Agent
# TEAM: NeXifyAI GTM
# WHAT: CLI — discover / status / export / promote / mail (dry-run|--send)
# WHY: One command weekly cadence for zero-cost acquisition agents
# BEST-PRACTICE: Default dry-run; --send requires researched+send_allowed
# PITFALL: V-LEAD-06: Never log secrets; prefer /etc/nexifyai/mail-nexifyai.env
...
def cmd_allow(args: argparse.Namespace) -> int:
    ...
    for lead in leads:
        ...
        if args.limit and n >= args.limit:
            break
    save_queue(q, leads)
    print(json.dumps({"ok": True, "allowed": n, "queue": str(q)}))
    return 0
...
def cmd_promote(args: argparse.Namespace) -> int:
    ...
    with out.open("a", encoding="utf-8") as fh:
        for lead in leads:
            ...
            n += 1
    print(json.dumps({"ok": True, "promoted": n, "out": str(out)}))
    return 0
...
def cmd_mail(args: argparse.Namespace) -> int:
    ...
    print(json.dumps({"ok": result.errors == 0, "mode": "send" if args.send else "dry-run",
                      "queue": str(queue_path(data_dir)), **result.as_dict()},
                     ensure_ascii=False, indent=2))
...
def build_parser() -> argparse.ArgumentParser:
    ...
    m = sub.add_parser("mail"); m.add_argument("--send", action="store_true"); m.add_argument("--limit", type=int, default=8)
