...
def scan() -> list[Finding]:
    ...
    # --- Model strategy: Upstage-first (Cutover 2026-07-27) ---
    cfg_text = (ROOT / "agent-config.yaml").read_text(encoding="utf-8") if (ROOT / "agent-config.yaml").exists() else ""
    if "primary_provider: upstage" in cfg_text:
        findings.append(Finding("ok", "MODEL-UPSTAGE", "agent-config.yaml: primary_provider upstage"))
    else:
        findings.append(
            Finding(
                "warn",
                "MODEL-DRIFT",
                "agent-config.yaml ohne primary_provider: upstage",
                "Upstage-first laut 9ROUTER Cutover 2026-07-27 setzen",
            )
        )
