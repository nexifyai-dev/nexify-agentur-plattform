...
## 2026-08-09 — CEO-Runde: WhatsApp-Ausfall behoben + Monitoring-Lücke geschlossen (E3)
- **Ausfall:** WhatsApp-Bridge seit ~22:00 08-08 „Logged out" (serverseitig getrennt), 11h unbemerkt — health-watchdog.py prüfte :3000 nicht.
- **Fix 1 (Monitoring):** health-watchdog.py ergänzt: `whatsapp_bridge_status()` (JSON `status=connected` nötig) + `was_down`-Bug gefixt (Recovery-Alarm war tot). Deploy Host, Test: `ALARM gesendet (1 Dienste)` via Telegram, Dedup ok. Backup: health-watchdog.py.bak-20260809.
- **Fix 1b (Health-Check-Retry):** `health-check.sh` (beide Timer, /opt + /workspace identisch via Bind-Mount) bekam Retry bei 000/5xx (2s warten, 2. Versuch 8s) — False-Positive bei Lastspitzen (AgentMemory:3113 → 000 war transient, live 3×200 <12ms). Test: `Health: 0/11 failures`, healthchecks.io OK. Backup: health-check.sh.bak-20260809.
- **Fix 2 (Pairing):** Session getrennt → neue leere Session angelegt; QR-Pairing-Loop als `systemd-run wa-pair-daemon` (bridge.js --pair-only --pair-json, 40×90s, auto `systemctl restart hermes-gateway` wenn creds.json erscheint). Alt-Session: session.loggedout-20260809-0913.
- **Host-Zugang (validiert E3):** SSH 2222 nexify-admin → Container; Host via `sudo docker run --rm --pid=host --privileged postgres:16-alpine nsenter -t 1 -m -p -u -n -- bash -c "echo <b64>|base64 -d|bash"` (base64 nötig, Quoting-Bug sonst). Helper: /tmp/nxh.sh.
- **Load 13→8:** 3 stale mcp-remote-Prozesse (31h alt) gekillt. headroom 89% CPU = legitimer Agent-Traffic (4 aktive :7777-Verbindungen), kein Bug.
- **OFFEN (wartet Pascal):** QR-Scan nötig (PNG in Session bereitgestellt). Danach: creds.json → auto Gateway-Restart → Bridge connected.
